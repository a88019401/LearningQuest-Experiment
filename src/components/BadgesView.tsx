...
function BadgePlanningPanel({
  ...
  const [retireId, setRetireId] = useState(""); // 🌟 改存 ID
  const [retireReason, setRetireReason] = useState(""); 
  const [retireNote, setRetireNote] = useState("");
  
  const [passOpen, setPassOpen] = useState(false); 
  const [passId, setPassId] = useState(""); // 🌟 改存 ID
  ...
  const closeWizard = () => { setIsOpen(false); setStep(0); setSelectedKey(""); };
  ...
  const openWizard = () => {
    ...
    if (last && !last.retired && safeTierOf(last.key) === 0) {
      setRetireId(last.id); setRetireReason(""); setRetireNote(""); setRetireOpen(true); return;
    }
    if (last && !last.retired && safeTierOf(last.key) >= 1 && !last.passReflectReason) {
      setPassId(last.id); setPassReason(""); setPassNote(""); setPassOpen(true); return;
    }
  ...
  const renderPlannedBadgeCard = (plan: BadgePlanConfig, index: number) => {
    ...
    return (
      <div key={id} className={`relative p-4 rounded-2xl border transition ${style} ${plan.retired ? "opacity-60" : ""}`}>
        ...
        <div className="absolute top-3 right-3 flex gap-2 z-20">
          <button onClick={() => { setPassId(id); setPassOpen(true); }} disabled={!needsReflect} className={`px-3 py-1 rounded-full text-xs font-semibold border ${needsReflect ? "border-black bg-white text-black hover:bg-neutral-50" : "opacity-30 cursor-not-allowed"}`}>反思</button>
          <button onClick={() => { setRetireId(id); setRetireOpen(true); }} disabled={!canRetire} className={`px-3 py-1 rounded-full text-xs font-semibold border ${canRetire ? "border-black bg-white text-black hover:bg-neutral-50" : "opacity-30 cursor-not-allowed"}`}>退休</button>
        </div>
        
        <div className="text-4xl mb-2 mt-6 text-center drop-shadow-sm">{icon}</div>
        <div className="font-extrabold text-center text-base mb-1 text-neutral-900">{plan.name}</div>
              ...
              {step === 0 && (
                <div className="grid grid-cols-2 gap-2">
                  {rows.map((r) => {
                    ...
                    const activePlan = plans.find(p => p.key === r.key && !p.retired);
                    const isDisabled = !!activePlan;
                    const statusText = isDisabled ? (safeTierOf(r.key) > 0 ? "已通關" : "進行中") : "";

                    return (
                      <button key={r.key} onClick={() => setSelectedKey(r.key)} disabled={isDisabled}
                      ...
                      >
                        <div className="font-bold text-sm flex justify-between items-center">
                          {r.name}
                          {statusText && <span className={`text-[10px] px-2 py-0.5 rounded-full font-normal ${selectedKey === r.key ? "bg-white/20" : "bg-black/10 text-neutral-800"}`}>{statusText}</span>}
                        </div>
              ...
              {step === 1 && currentRow && (
                <div className="space-y-4">
                  <div className="p-3 bg-indigo-50 border border-indigo-200 rounded-xl text-sm text-indigo-900">
                    <strong>歷史提示：</strong>
                    {currentRow.key === "SPEED_DEMON" ? historicalBest > 0 ? `你目前的最佳紀錄是 ${historicalBest} 秒，挑戰自己吧！` : "你還沒玩過挑戰區，先預設一個秒數目標吧！" : `這個任務會從你按下設定的這一刻「從 0 開始」累積計算喔！`}
                  </div>
                  <p className="text-sm font-bold">請輸入你要挑戰的數字：</p>
                  <div className="grid grid-cols-3 gap-3">
                    <div><label className="block text-xs text-amber-700 font-bold mb-1">銅級</label><input type="number" value={currentRow.thresholds.bronze || ""} onChange={(e) => updateRow(currentRow.key, { thresholds: { ...currentRow.thresholds, bronze: Number(e.target.value) }})} className="border p-2 w-full rounded-xl outline-none focus:ring-2 focus:ring-amber-400" /></div>
                    <div><label className="block text-xs text-slate-500 font-bold mb-1">銀級</label><input type="number" value={currentRow.thresholds.silver || ""} onChange={(e) => updateRow(currentRow.key, { thresholds: { ...currentRow.thresholds, silver: Number(e.target.value) }})} className="border p-2 w-full rounded-xl outline-none focus:ring-2 focus:ring-slate-400" /></div>
                    <div><label className="block text-xs text-yellow-600 font-bold mb-1">金級</label><input type="number" value={currentRow.thresholds.gold || ""} onChange={(e) => updateRow(currentRow.key, { thresholds: { ...currentRow.thresholds, gold: Number(e.target.value) }})} className="border p-2 w-full rounded-xl outline-none focus:ring-2 focus:ring-yellow-400" /></div>
                  </div>
      ...
      {retireOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="relative bg-white p-6 rounded-2xl w-full max-w-md shadow-xl">
            ...
            <h3 className="font-bold text-lg mb-1 text-red-700">退休這枚獎章</h3>
            <p className="text-sm text-neutral-600 mb-4">誠實面對自己也是一種學習，請記錄下退休的原因供後續研究分析。</p>
            ...
            <textarea value={retireNote} onChange={(e) => setRetireNote(e.target.value)} className={`w-full border p-3 mb-1 rounded-xl bg-neutral-50 outline-none focus:ring-2 ${retireNote.trim().length >= 8 ? "focus:ring-green-400 border-green-200" : "focus:ring-red-200"}`} placeholder="補充說明 (例如：我原本以為每天10次很容易，但...)" rows={3} />
      ...
      {passOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="relative bg-white p-6 rounded-2xl w-full max-w-md shadow-xl">
            ...
            <textarea value={passNote} onChange={(e) => setPassNote(e.target.value)} className={`w-full border p-3 mb-1 rounded-xl bg-neutral-50 outline-none focus:ring-2 ${passNote.trim().length >= 8 ? "focus:ring-green-400 border-green-200" : "focus:ring-green-200"}`} placeholder="補充說明 (例如：我每天固定睡前玩兩場，效果很好...)" rows={3} />
